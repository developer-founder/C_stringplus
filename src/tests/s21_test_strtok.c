...
START_TEST(test_multiple_delimiters) {
  compare_tokens("one,two;three:four", ",;:");
}
END_TEST

START_TEST(test_leading_trailing_delims) {
  compare_tokens("  hello world  ", " ");
}
END_TEST

START_TEST(test_consecutive_delimiters) {
  compare_tokens("one,,,two,,three", ",");
}
END_TEST
